      ...
      <main className="mx-auto max-w-6xl px-4 py-12 sm:px-6 lg:px-8 relative z-10">
          ...
          <div className="bg-card/60 backdrop-blur-sm border border-primary/30 p-6 rounded-xl shadow-lg flex items-center gap-6 min-w-[240px]">
            ...
            <div>
              ...
              <div className="flex items-center gap-2">
                <span className="text-3xl font-mono font-bold text-primary">{userStats.sparks}</span>
                <span className="text-sm font-semibold text-muted-foreground">Sparks</span>
              </div>
            </div>
          </div>
        </div>
          ...
          {rewards.map((reward) => (
            ...
            >
              <div className="flex items-start justify-between mb-4">
                ...
                <Badge variant="secondary" className="bg-muted/50 text-muted-foreground text-[10px] font-bold uppercase tracking-wider">
                  {reward.category}
                </Badge>
              </div>
              ...
              <div className="space-y-2 mb-6">
                ...
                <p className="text-xs font-semibold text-primary/80 uppercase tracking-tighter">{reward.provider}</p>
              ...
              <div className="flex items-center justify-between pt-4 border-t border-border/30">
                <div className="flex items-center gap-1.5">
                  ...
                  <span className="text-lg font-mono font-bold">{reward.cost}</span>
                </div>
                
                <Button 
                  onClick={() => handleRedeem(reward.id, reward.cost)}
                  disabled={userStats.sparks < reward.cost || redeemedId === reward.id}
                  className={`
              ...
              {/* Decorative scanline effect for redeemed items */}
              {redeemedId === reward.id && (
        ...
        <section className="mt-20 p-8 rounded-2xl bg-muted/20 border border-border/50 text-center space-y-4">
          ...
          <p className="text-muted-foreground max-w-2xl mx-auto leading-relaxed">
            ...
            When you redeem, we verify your activity and compensate the providers directly. 
            Keep on questing to keep the community thriving!
          </p>
        </section>
      </main>
