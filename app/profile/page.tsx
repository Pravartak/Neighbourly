...
const userProfile = {
  ...
  tags: ["🛠️ Handy", "💻 Tech Savvy", "📦 Delivery Pro"]
}
...
export default function ProfilePage() {
  ...
  const addedQuests = quests.filter(q => q.isUserAdded)
            ...
            <div className="flex-1 space-y-4">
              <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                  ...
                  <p className="text-muted-foreground italic">@{userProfile.username}</p>
                </div>
              ...
              <div className="space-y-2">
                <div className="flex items-center justify-between items-end">
                  <p className="max-w-xl text-md leading-relaxed text-muted-foreground">{userProfile.bio}</p>
                  <div className="text-right hidden md:block">
                    <span className="text-xs font-bold text-primary block uppercase tracking-wider mb-1">Experience</span>
                    <span className="text-sm font-mono">{userStats.xp} / {userStats.xpToNextLevel} XP</span>
                  </div>
                </div>
              </div>
        ...
        <section className="space-y-6">
          ...
          <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
            <TabsList className="bg-muted/50 border border-border/50 p-1 mb-8 w-full sm:w-auto h-auto sm:h-10 flex-wrap sm:flex-nowrap">
              <TabsTrigger value="in-progress" className="flex-1 sm:flex-none flex items-center gap-2 px-6">
                <Timer className="h-4 w-4" />
                In Progress
                {inProgressQuests.length > 0 && (
                  <Badge variant="secondary" className="bg-primary text-primary-foreground ml-2 px-1.5 py-0 min-w-5 h-5 flex items-center justify-center rounded-full text-[10px]">
                    {inProgressQuests.length}
                  </Badge>
                )}
              </TabsTrigger>
              <TabsTrigger value="added" className="flex-1 sm:flex-none flex items-center gap-2 px-6">
              ...
              <TabsTrigger value="completed" className="flex-1 sm:flex-none flex items-center gap-2 px-6">
            ...
            <TabsContent value="in-progress" className="space-y-4">
              <QuestList quests={inProgressQuests} emptyMsg="No quests currently in progress." showBrowse />
            </TabsContent>

            <TabsContent value="added" className="space-y-4">
              <QuestList quests={addedQuests} emptyMsg="You haven't posted any quests yet." />
            </TabsContent>

            <TabsContent value="completed" className="space-y-4">
              <QuestList quests={completedQuests} emptyMsg="No completed quests yet. Start your first mission!" showBrowse />
            </TabsContent>
          </Tabs>
        </section>
      </main>
...
function QuestList({ quests, emptyMsg, showBrowse }: { quests: any[], emptyMsg: string, showBrowse?: boolean }) {
  if (quests.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-16 text-center border-2 border-dashed border-border/50 rounded-lg bg-muted/10">
        ...
        {showBrowse && (
          <Link href="/quests">
            <Button variant="link" className="text-primary mt-2">Browse for a new adventure</Button>
          </Link>
        )}
      </div>
    )
