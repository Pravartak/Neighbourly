	...
	const categories = useMemo(
		() => [
			{ id: "all", name: "All Quests", icon: null, count: quests.length },
			{
				...
				name: "Errands",
				icon: ShoppingBag,
				count: quests.filter((quest) => quest.category === "Errands").length,
			},
			{
				id: "Nature",
				name: "Nature",
				icon: TreePine,
				count: quests.filter((quest) => quest.category === "Nature").length,
			},
			{
				id: "Creative",
				name: "Creative",
				icon: Palette,
				count: quests.filter((quest) => quest.category === "Creative").length,
			},
			{
				id: "Learn",
				name: "Learn & Teach",
				icon: GraduationCap,
				count: quests.filter((quest) => quest.category === "Learn").length,
			},
			{
				id: "Urgent",
				name: "Urgent",
				icon: AlertTriangle,
				count: quests.filter((quest) => quest.category === "Urgent").length,
			},
		],
		[quests],
	);
					...
					{filteredQuests.length > 0 ? (
						...
						<div className="text-center py-20 px-4">
							...
							<p className="text-muted-foreground mb-6 max-w-md mx-auto">
								We could not find any quests matching your filters. Try
